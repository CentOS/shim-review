...
RUN yum install -y binutils --disablerepo=updates
RUN yum install -y gcc gnu-efi gnu-efi-devel make redhat-rpm-config-9.1.0-80.el7 rpm-build yum-utils wget --disablerepo=updates
...
RUN sha256sum /build/builddir/BUILDROOT/shim-12-2.el7.*/usr/share/shim/*-12-2.el7/shim*.efi
