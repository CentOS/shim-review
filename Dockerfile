...
RUN rpm -ivh shim-unsigned-x64-15.8-3.el10.centos.src.rpm
...
COPY shimx64.efi /
RUN rpm2cpio /builddir/build/RPMS/x86_64/shim-unsigned-x64-15.8-3.el10.centos.x86_64.rpm | cpio -diu
RUN ls -l /*.efi ./usr/share/shim/15.8-3.el10.centos/*/shim*.efi
RUN hexdump -Cv ./usr/share/shim/15.8-3.el10.centos/x64/shimx64.efi > built-x64.hex
RUN hexdump -Cv /shimx64.efi > orig-x64.hex
RUN objdump -h ./usr/share/shim/15.8-3.el10.centos/x64/shimx64.efi
RUN diff -u orig-x64.hex built-x64.hex
RUN pesign -h -P -i ./usr/share/shim/15.8-3.el10.centos/x64/shimx64.efi
RUN pesign -h -P -i /shimx64.efi
RUN sha256sum ./usr/share/shim/15.8-3.el10.centos/x64/shimx64.efi /shimx64.efi
